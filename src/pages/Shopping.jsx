  ...
  return <AppLayout>
    ...
    {tab === "basket" ? <div className="space-y-4">
      <section className="rounded-2xl border border-primary/25 bg-primary/[0.07] p-4 shadow-[0_0_28px_rgba(0,220,230,0.07)]">
        <div className="flex items-start justify-between gap-3"><div><div className="mb-1 flex items-center gap-2 text-primary"><Sparkles className="h-4 w-4" /><span className="text-xs font-bold uppercase tracking-[0.14em]">Price intelligence</span></div><h2 className="text-lg font-bold">{bestStore ? `Best current match: ${bestStore.store}` : "Building your price basket"}</h2><p className="mt-1 text-xs leading-5 text-muted-foreground">Every recognised ingredient is now mapped to a product family. We only show a price when the pack price has been verified; unmatched prices are never invented.</p></div>{bestStore && <div className="text-right"><p className="text-2xl font-black text-primary">{money(bestStore.total)}</p><p className="text-[10px] text-muted-foreground">priced subtotal</p></div>}</div>
