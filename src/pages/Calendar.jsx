...
export default function Calendar() {
  ...
  return <AppLayout>
    ...
    <div className="space-y-2">{Array.from({ length: 7 }, (_, i) => {
      ...
      return <button key={dateStr} type="button" onClick={() => navigate(`/train?day=${((weekday + 6) % 7)}`)} className={cn("flex w-full items-center gap-3 rounded-2xl border p-4 text-left transition-colors hover:bg-secondary/40", dateStr === todayDate ? "border-primary/50 bg-primary/5" : "border-border bg-card")} aria-label={`Open training for ${WEEKDAY_LABELS[weekday]}`}>
