    ...
    <AppLayout>
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Manage your ShiftFit preferences</p>
        </div>
      </div>

      {sections.map((section) => (
        <section key={section.title} className="mb-5">
          <h2 className="mb-2 px-1 text-[10px] font-extrabold tracking-[0.18em] text-muted-foreground">
            {section.title}
          </h2>
            ...
            {section.items.map((item, index) => {
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center gap-3 px-4 py-3.5 text-left transition-colors hover:bg-secondary/50 ${index < section.items.length - 1 ? "border-b border-border" : ""}`}
                >
      ...
      <section className="mb-5">
        ...
        <button
          type="button"
          onClick={logout}
          className="flex w-full items-center justify-center gap-2 rounded-2xl border border-border bg-card px-4 py-3.5 text-sm font-bold text-destructive transition-colors hover:bg-destructive/10"
        >
