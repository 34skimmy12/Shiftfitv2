...
import { weekdayOf, WEEKDAY_LABELS, todayStr, swapMeal, generateShoppingList, computeTargets } from "@/lib/fitnessUtils";
...
export default function Nutrition() {
  ...
  const loadData = async () => {
    if (!profile) return;
    const [mealPlans, waterLogs, basket] = await Promise.all([
  ...
  const handleSwap = async (mealIndex) => {
    ...
    try {
      ...
      const updatedPlan = swapMeal(plan, mealIndex, profile, targets);
      const saved = await base44.entities.MealPlan.update(plan.id, {
        meals: updatedPlan.meals,
        total_calories: updatedPlan.total_calories,
        total_protein: updatedPlan.total_protein,
      });
