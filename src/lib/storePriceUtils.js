...
// Never invent a price: an empty store entry means price verification is still pending.
...
const STORE_PRICES = {
  ...
  "turkey slices": { unit:"g", pack:100, stores:{ Tesco:{price:1.90} } },
  "smoked salmon": { unit:"g", pack:100, stores:{ Tesco:{price:3.80} } },
  ...
  tomatoes: { unit:"g", pack:1, stores:{ Tesco:{price:0.99,pack:1} }, flexibleUnit:true },
  ...
  peppers: { unit:"g", pack:1, stores:{ Tesco:{price:1.50,pack:1} }, flexibleUnit:true },
  onions: { unit:"g", pack:1, stores:{ Tesco:{price:0.11,pack:1} }, flexibleUnit:true },
...
// Permanent aliases used by meal generation, swaps and basket parsing.
const ALIASES = [
  ["chicken", "chicken breast"], ["turkey mince", "turkey mince"], ["turkey", "turkey mince"],
  ["lean beef", "beef mince"], ["beef", "beef mince"], ["smoked salmon", "smoked salmon"], ["salmon", "salmon"],
  ["tuna", "tuna"], ["cod", "cod"], ["turkey slices", "turkey slices"], ["whey", "whey protein"],
  ["egg", "eggs"], ["eggs", "eggs"], ["greek yogurt", "greek yogurt"], ["greek yoghurt", "greek yogurt"],
  ["cottage cheese", "cottage cheese"], ["skyr", "skyr"], ["milk", "milk"], ["oats", "oats"],
  ["brown rice", "rice"], ["basmati rice", "basmati rice"], ["rice", "rice"], ["quinoa", "quinoa"],
  ["baby potatoes", "potatoes"], ["potatoes", "potatoes"], ["sweet potato", "sweet potatoes"], ["toast", "wholegrain bread"],
  ["wholegrain bread", "wholegrain bread"], ["wholemeal bread", "wholegrain bread"], ["wrap", "wholegrain wraps"],
  ["wholegrain wrap", "wholegrain wraps"], ["hummus", "hummus"], ["houmous", "hummus"], ["berries", "mixed berries"],
  ["berry", "mixed berries"], ["banana", "banana"], ["apple", "apple"], ["pear", "pear"], ["avocado", "avocado"],
  ["spinach", "spinach"], ["broccoli", "broccoli"], ["green beans", "green beans"], ["salad leaves", "salad leaves"],
  ["tomato", "tomatoes"], ["mixed vegetables", "mixed vegetables"], ["stir-fry", "stir-fry vegetables"],
  ["pepper", "peppers"], ["onion", "onions"], ["almonds", "almonds"], ["peanut butter", "peanut butter"],
  ["chia", "chia seeds"], ["pumpkin seeds", "pumpkin seeds"], ["sesame oil", "sesame oil"], ["olive oil", "olive oil"], ["honey", "honey"],
];
...
export function expandStorePriceMatch(item) {
  ...
  const required = toBaseAmount(
    item?.priceMatch?.required ?? item?.amount,
    item?.priceMatch?.unit ?? item?.unit,
    catalog.unit
  );
  if (!Number.isFinite(required)) return item;

  const offers = Object.entries(catalog.stores || {}).map(([store, spec]) => {
    const pack = Number(spec.pack || catalog.pack);
    const packPrice = Number(spec.price);
    const packs = Math.max(1, Math.ceil(required / pack));
    return {
      store, packs,
      total: Number((packs * packPrice).toFixed(2)),
      packPrice: Number(packPrice.toFixed(2)), pack,
      unit: catalog.unit, source: src, checkedAt: D,
    };
  });
  ...
  return {
    ...item,
    priceMatch: {
      ...(item.priceMatch || {}), key, required,
      offers: allOffers, cheapest, productMatch: true,
      source: src, checkedAt: D,
    },
  };
}
...
export function getStorePriceDatabaseStats() {
  const families = Object.keys(STORE_PRICES);
  const priced = families.filter(key => Object.keys(STORE_PRICES[key].stores || {}).length);
